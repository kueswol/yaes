...
async fn handle_socket(mut socket: WebSocket, world: Arc<Mutex<World>>) {
    loop {
        ...
        let mut buffer = Vec::with_capacity(creatures.len()*8); // "* 8" because each creature has 2 f32 values (x and y) which are 4 bytes each
        for c in creatures {
        ...
        // let json = serde_json::to_string(&creatures).unwrap();
